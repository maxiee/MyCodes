...
for loop = 1:Loop
    ...
    for i=1:count
        ...
        % MAUKF
        [M_maukf, P_maukf, D_maukf, sigma_an, sigma_ay, wei_ut] = ukf_predict1(M_maukf, P_maukf, f, Qv, dt, 1, 1, -2);
        % UKF 原始更新过程不用, 用的是扩展的增广更新
        % [M_maukf, P_maukf] = ukf_update1(M_maukf, P_maukf, [zx; zy], h, Qw);
        y_ag = augmentation_h(sigma_an, dt); % Sigma点增广h非线性变换
        hat_y_ag = y_ag * wei_ut{1}; % 得到k时刻先验估计
        Pyy_maukf = diag([25, 25, 10^-9]);
        Pxy_maukf = 0;
        for j = 1:11
            Pyy_maukf = Pyy_maukf + wei_ut{2}(j)*(y_ag(:, i) - hat_y_ag)*(y_ag(j) - hat_y_ag)';
